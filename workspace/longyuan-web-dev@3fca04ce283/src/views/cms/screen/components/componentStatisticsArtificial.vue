<template>
  <div class="device-status sceen-element-unit">
    ...
    <div class="header-top">
      ...
      <span class="sceen-title-text">{{ $t('screen.componentStatisticsArtificial') }}</span>
    </div>
    <div id="artificial-chart"></div>
  </div>
</template>
...
export default {
  name: 'componentStatisticsArtificial',
  props: {
    windowResize: {
      type: Boolean,
      default: false,
  ...
  mounted() {
    this.chart = echarts.init(document.getElementById('artificial-chart'));
    this.chart.resize();
    window.addEventListener('resize', this.onWindowResize, false);
    this.getComponentFailure(this.$route.query.hierarchyId);
  },
  beforeDestroy() {
  ...
  methods: {
    getComponentFailure(id) {
      ...
      this.$api.screen.getComponentFailure(id).then((res) => {
