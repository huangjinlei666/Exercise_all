<template>
  <div class="fault-device-brand sceen-element-unit">
    ...
    <div class="header-top">
      ...
      <span class="sceen-title-text">{{ $t('screen.faultyEquipmentTop5') }}</span>
    </div>
    <div id="fault-device-brand-chart"></div>
  </div>
</template>
...
export default {
  ...
  mounted() {
    this.chart = echarts.init(document.getElementById('fault-device-brand-chart'));
    this.chart.resize();
  ...
  methods: {
    initData() {
      ...
      this.fetchBrandFailure(hierarchyId);
    },
    // 获取故障设备品牌top5
    fetchBrandFailure(id) {
      if (!id) {
        return;
      }
