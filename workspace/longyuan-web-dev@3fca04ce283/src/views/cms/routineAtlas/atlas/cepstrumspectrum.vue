<template>
  <div class="charts-box">
    ...
    <span class="chart-box-title">{{ $t('routineAtlas.selectTypes.cepstrumSpectrum') }}</span>
  ...
  methods: {
    ...
    request(param) {
      ...
      this.$api.routineAtlas
        .getCepstrumSpectrumMap(sensorId, params)
        .then((res) => {
          let xAxis = [];
          let yAxis = [];
          let textObj = {};
          if (!res || !res.data || res.data.length === 0) {
            ...
            if (res.data[0].timeDelta && res.data[0].timeDelta.length > 0) {
              xAxis = res.data[0].timeDelta.splice(1);
            }
            if (res.data[0].amplitude && res.data[0].amplitude.length > 0) {
    ...
    computedSubscribeUrl(sensorId) {
      return `/ws/${sensorId}/cepstrumSpectrum`;
    },
    connectStomp(param) {
      if (!this) return;
    ...
    subscribe(url, param) {
      if (!this) return;
      const { sensorName, dataType, extensionType } = param;
      this.subscription = this.stomp.subscribe(url, this, (data, sub) => {
        ...
        let yAxis = [];
        // 频谱图
        if (body.timeDelta && body.timeDelta.length > 0) {
          xAxis = body.timeDelta.splice(1);
        }
    ...
    draw(response) {
      ...
      const chartTitleText = this.$t('routineAtlas.selectTypes.cepstrumSpectrum');
      ...
      const option = {
        ...
        tooltip: {
          ...
          formatter(params) {
            return `${xAxisText}: ${isValueExist(params[0].value[0]) ? params[0].value[0] : ''} s<br />
