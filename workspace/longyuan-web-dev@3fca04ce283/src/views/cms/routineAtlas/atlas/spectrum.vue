<template>
  <div class="charts-box" ref="chartBox">
    <stft ref="stft" v-if="isShowSTFT" :requestParam="requestParam" :inputValue="inputValue" />
    <div v-show="!isShowSTFT" class="myChart" :ref="sid"></div>
    <span class="chart-box-title">{{ $t('routineAtlas.selectTypes.spectrum') }}</span>
    <div class="selecet-box box" v-if="!isShowSTFT" :style="isCompare && { left: '60px' }">
      <span class="mr-5">{{ $t('routineAtlas.functionType') }}</span>
      <el-select
        v-model="windowType"
        :placeholder="$t('routineAtlas.placeholder.functionType')"
        size="mini"
        class="width-95"
        @change="windowTypeChange"
      >
        <el-option v-for="item in functionTypes" :key="item.value" :label="$t(item.label)" :value="item.value">
        </el-option>
      </el-select>
    </div>
    <div class="selecet-box2 box" v-if="!isShowSTFT" :style="isCompare && { left: '160px' }">
      <span class="mr-5">{{ $t('routineAtlas.averageTimes') }}</span>
      <el-select
        v-model="averageTime"
        :placeholder="$t('routineAtlas.placeholder.averageTimes')"
        size="mini"
        class="width-60"
        @change="timesChange"
      >
        <el-option v-for="item in ['1', '2', '4', '8']" :key="item" :label="item" :value="item"> </el-option>
      </el-select>
    </div>
    <template v-if="isShowSTFT">
      <div v-for="item in inputList" :key="item.key" class="box" :class="item.key">
        <span class="mr-5">{{ item.name }}</span>
        <el-input class="width-100" size="mini" v-model="item.value" :placeholder="`请输入${item.name}`"></el-input>
      </div>
      <el-button type="primary" size="mini" @click="searchInput" class="input-search box">{{
        $t('common.search')
      }}</el-button>
    </template>
    ...
    <template v-if="edgeDataArr.length">
      <div class="sideband-content">
        <p>Sideband Values</p>
        <ul>
          <li v-for="(item, index) in edgeDataArr" :key="index" :style="{ color: item.isCenter ? '#f00' : '#1890ff' }">
            <el-row>
...
import { functionTypes } from '@/util/selectTypes';
import mixin from './mixin';
import ZoomSpectrumModal from '../modal/zoomSpectrumModal.vue';
import BasicSetModal from '../modal/basicSetModal.vue';
import stft from './spectrumSTFT.vue';
...
export default {
  ...
  data() {
    return {
      ...
      edgeInterval: 0,
      // 细化谱
      isZoomSelect: false, // 是否在选取范围（细化谱需求）
  ...
  watch: {
    requestParam(value, oldValue) {
      if (!this) return;
      if (value.xAxisUnit === 'STFT') {
        this.isShowSTFT = true;
        this.$nextTick(() => this.$refs.stft.renderChart(value));
      } else {
        this.isShowSTFT = false;
        this.clearDraw();
  ...
  methods: {
    ...
    request(param, obj = {}) {
      ...
      const params = {
        ...
        type: xAxisUnit,
        extensionType,
        identifier: identifier === 'longOriginalData' ? 'longOriginalData' : null,
        // identifierOriginalDataType,
        windowFlag: this.windowType,
      ...
      this.$api.routineAtlas
        ...
        .then((res) => {
          ...
          if (!res || !res.data || res.data.length === 0) {
            ...
            if (res.data[0] && res.data[0].frequency && res.data[0].frequency.length > 0) {
              xAxis = res.data[0].frequency.splice(1);
            }
            if (res.data[0] && res.data[0].order && res.data[0].order.length > 0) {
              xAxis = res.data[0].order.splice(1);
            }
            if (res.data[0] && res.data[0].amplitude && res.data[0].amplitude.length > 0) {
              yAxis = res.data[0].amplitude.splice(1);
            }
            textObj = {
              sensorName,
              identifierName,
    ...
    subscribe(url, param) {
      ...
      this.subscription = this.stomp.subscribe(url, this, (data, sub) => {
        ...
        if (body.frequency && body.frequency.length > 0) {
          xAxis = body.frequency.splice(1);
        }
        if (body.order && body.order.length > 0) {
          xAxis = body.order.splice(1);
        }
        if (body.amplitude && body.amplitude.length > 0) {
          yAxis = body.amplitude.splice(1);
        }
        if (!this.staticflag) {
          this.draw({
            xAxis,
    ...
    draw(response, rotingVelocity) {
      ...
      const chartTitleText = this.$t('routineAtlas.selectTypes.spectrum');
    ...
    getZoom() {
      ...
      const params = {
        startAt: this.requestParam.startAt,
        endAt: this.requestParam.endAt,
        startFrequency,
        endFrequency,
      };
      const winData = {
      ...
      this.$modal.show(this.$t('routineAtlas.selectTypes.zoomSpectrum'), ZoomSpectrumModal, winData, '800px');
    },
    // ====== 细化谱逻辑 end ======
    // ====== 倍频逻辑 begin ======
    onSingleClick(e) {
      ...
      if (option.series[0].data.length === 0) return false;
      const pos = this.chart.convertFromPixel('grid', [e.offsetX, e.offsetY]);
      const [x, y] = this.calculateXY(pos[0]);
      this.singleCursor = { x: x.toFixed(2), y: y.toFixed(6) };
      this.resetMarkLine();
    },
    onContextMenu(e) {
      e && e.stop && e.stop();
