      ...
      <div v-loading="loading">
        ...
        <div class="user-table">
          <el-table :data="list" style="width: 100%">
            <el-table-column prop="bearingNumber" :label="$t('bearing.bearingNumber')"></el-table-column>
            <el-table-column prop="statusText" :label="$t('common.status')"></el-table-column>
  ...
  methods: {
    deleteUser: debounce(
      function deleteUser() {
        this.loading = true;
        this.$api.bearing
          .deleteBearing(this.id)
          .then((res) => {
            if (!res || !res.success) {
              this.$msg.error(this.$t('bearing.failure.deleteBearing'));
              return;
            }
            this.$msg.success(this.$t('bearing.success.deleteBearing'));
            this.handleConfirm();
          })
          .catch((err) => {
            this.$msg(err);
