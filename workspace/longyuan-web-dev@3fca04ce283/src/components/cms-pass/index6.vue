...
export default {
  ...
  mounted() {
    ...
    if (this.selectedNodes) {
      Object.keys(this.selectedNodes).forEach((key) => {
        keys.push(this.selectedNodes[key]);
      });
      this.selectedNodeKey = this.selectedNodes.MACHINE
        ? this.selectedNodes.MACHINE
        : this.selectedNodes.WORKSHOP
        ? this.selectedNodes.WORKSHOP
        : this.selectedNodes.FACTORY
        ? this.selectedNodes.FACTORY
        : this.passList[0].id;
      this.$nextTick(() => {
        this.defaultExpandedKeys = [...this.defaultExpandedKeys, ...keys, this.passList[0].id];
  ...
  beforeDestroy() {
    ...
    this.saveSelectedNodes({ selectedNodes: {} });
  },
  watch: {
    passList: {
      handler(val) {
  ...
  methods: {
    ...
    //   arr.forEach((i) => {
    //     result.push(i.id);
    //     if (i.hierarchies.length > 0 && i.type === 'PROJECT') {
    //       const childResult = this.getDefaultExpandedKeys(i.hierarchies);
    //       result = [...result, ...childResult];
    //     }
    //     // if (i.hierarchies.length > 0 && (i.type === 'PROJECT' || i.type === 'FACTORY')) {
    //     if (i.hierarchies.length > 0 && i.type === 'WORKSHOP' && i.id === this.selectedNodes.workshop) {
    //       const childResult = this.getDefaultExpandedKeys(i.hierarchies);
    //       result = [...result, ...childResult];
    //     }
    //     if (i.hierarchies.length > 0 && i.type === 'FACTORY' && i.id === this.selectedNodes.factory) {
    //       const childResult = this.getDefaultExpandedKeys(i.hierarchies);
    //       result = [...result, ...childResult];
    //     }
    //   });
